    ...
    fs.createReadStream("./cron/businessGotoMarket.csv")
        ...
        try {
            ...
            for (const business of results) {
                const businessKey = `${business.name.toLowerCase()}-${business.longitude}`;
                // Skip if we've already processed this business at this location
                if (processedBusinesses.has(businessKey)) {
                    console.log(`Skipping duplicate business at same location: ${business.name}`);
                    continue;
                }
                ...
                console.log(`Created business: ${business.name} at ${business.longitude},${business.latitude}`);
            }
        }
        catch (error) {
...
async function createUser(userData) {
    try {
        ...
        const isUserRegistered = await db.User.findOne(payload);
        if (isUserRegistered) {
...
async function createBusiness(businessData) {
    const { userId, name, type, address, longitude, latitude, zone, logo } = businessData;
    try {
...
function generateEmail(businessName, allBusinesses = [], forceUnique = false) {
    ...
    const existingEmails = new Set(allBusinesses.map((b) => b.email).filter(Boolean));
